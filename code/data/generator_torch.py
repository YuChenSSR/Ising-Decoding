...
class QCDataGeneratorTorch:
    ...
    def __init__(
        ...
        **_ignored,
        ...
        if bool(decompose_y):
            raise ValueError("decompose_y is not supported in the Torch-only generator (set decompose_y=false).")
        if noise_model is not None:
            ...
            raise ValueError("noise_model is not supported in the Torch-only generator (simple single-p only).")
        ...
        if precomputed_frames_dir is None:
            ...
            if self.verbose:
                print(f"[QCDataGeneratorTorch] precomputed_frames_dir=None -> building in-memory DEM bundle at p={p_nom}")
        ...
        if self._mixed:
            self.sim_X = MemoryCircuitTorch(
                ...
                precomputed_frames_dir=precomputed_frames_dir, code_rotation=self.code_rotation,
                timelike_he=timelike_he, num_he_cycles=num_he_cycles, max_passes_w1=max_passes_w1, device=self.device,
            ...
            self.sim_Z = MemoryCircuitTorch(
                distance=self.distance, n_rounds=self.n_rounds, basis="Z",
                precomputed_frames_dir=precomputed_frames_dir, code_rotation=self.code_rotation,
                timelike_he=timelike_he, num_he_cycles=num_he_cycles, max_passes_w1=max_passes_w1, device=self.device,
            ...
            self.sim = MemoryCircuitTorch(
                distance=self.distance, n_rounds=self.n_rounds, basis=self._single_basis,
                precomputed_frames_dir=precomputed_frames_dir, code_rotation=self.code_rotation,
                timelike_he=timelike_he, num_he_cycles=num_he_cycles, max_passes_w1=max_passes_w1, device=self.device,
        ...
        if self.verbose:
            ...
            print(f"[QCDataGeneratorTorch] Initialized (d={self.distance}, r={self.n_rounds}, basis={b}, device={self.device})")
